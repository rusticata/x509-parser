...
#[test]
fn parse_certificate_igca() {
    let empty = &b""[..];
    let (rem, cert) = parse_x509_certificate(IGCA_DER).expect("X.509 parsing failed");
    // println!("res: {:?}", res);
    ...
    let expected_extensions = vec![
        ...
        X509Extension::new(
            ...
            ParsedExtension::SubjectKeyIdentifier(SubjectKeyIdentifier(KeyIdentifier(&[
                163, 5, 47, 24, 96, 80, 194, 137, 10, 221, 43, 33, 79, 255, 142, 78, 168, 48, 49,
                54,
            ]))),
        ),
        X509Extension::new(
            ...
            Input::new(
                &[
                    48, 22, 128, 20, 163, 5, 47, 24, 96, 80, 194, 137, 10, 221, 43, 33, 79, 255,
                    142, 78, 168, 48, 49, 54,
                ],
                Range {
                    start: 730,
            ...
            ParsedExtension::AuthorityKeyIdentifier(AuthorityKeyIdentifier {
                key_identifier: Some(KeyIdentifier(&[
                    163, 5, 47, 24, 96, 80, 194, 137, 10, 221, 43, 33, 79, 255, 142, 78, 168, 48,
                    49, 54,
                ])),
...
#[test]
fn parse_certificate_without_extensions() {
    let empty = &b""[..];
...
fn parse_subject_public_key_info() {
    let res = SubjectPublicKeyInfo::parse_der(Input::from(&IGCA_DER[339..]))
    ...
    // println!("spk.data.len {}", spk.as_bitslice().len());
    assert_eq!(spk.as_bitslice().len(), 2160);
}
...
#[test]
fn parse_crl() {
    let (rem, cert) = parse_x509_crl(CRL_DER).expect("CRL parsing failed");
    assert!(rem.is_empty());
    ...
    let expected_extensions = vec![
        X509Extension::new(
            ...
            Input::new(
                &[
                    48, 22, 128, 20, 190, 18, 1, 204, 170, 234, 17, 128, 218, 46, 173, 178, 234,
                    199, 181, 251, 159, 249, 173, 52,
                ],
                Range {
                    start: 480,
                    end: 504,
            ...
            ParsedExtension::AuthorityKeyIdentifier(AuthorityKeyIdentifier {
                key_identifier: Some(KeyIdentifier(&[
                    190, 18, 1, 204, 170, 234, 17, 128, 218, 46, 173, 178, 234, 199, 181, 251, 159,
                    249, 173, 52,
                ])),
                authority_cert_issuer: None,
                authority_cert_serial: None,
...
#[test]
fn parse_crl_empty() {
    let (rem, cert) = parse_x509_crl(EMPTY_CRL_DER).expect("CRL parsing failed");
    assert!(rem.is_empty());
    ...
    let expected_extensions = vec![
        ...
        X509Extension::new(
            ...
            Input::new(
                &[
                    48, 22, 128, 20, 34, 101, 12, 214, 90, 157, 52, 137, 243, 131, 180, 149, 82,
                    191, 80, 27, 57, 39, 6, 172,
                ],
                Range {
                    start: 183,
                    end: 207,
            ...
            ParsedExtension::AuthorityKeyIdentifier(AuthorityKeyIdentifier {
                key_identifier: Some(KeyIdentifier(&[
                    34, 101, 12, 214, 90, 157, 52, 137, 243, 131, 180, 149, 82, 191, 80, 27, 57,
                    39, 6, 172,
                ])),
                authority_cert_issuer: None,
                authority_cert_serial: None,
...
#[test]
fn parse_crl_minimal() {
    let (rem, crl) = parse_x509_crl(MINIMAL_CRL_DER).expect("CRL parsing failed");
...
fn test_duplicate_authority_info_access() {
    let (rem, cert) = parse_x509_certificate(DUPLICATE_VALUE_IN_AIA).expect("X.509 parsing failed");

    // Note: in this file, there are 2 extra bytes at end (00 00)
    assert_eq!(rem.len(), 2);
    ...
    if let ParsedExtension::AuthorityInfoAccess(aia) = extension.parsed_extension() {
        ...
        panic!("Extension not parsed");
    }
}

#[test]
fn parse_certificate_parse_opt_no_extensions() {
    let mut parser = X509CertificateParser::new().with_deep_parse_extensions(false);
    let (_, x509) = parser.parse(Input::from(IGCA_DER)).expect("parsing failed");
...
fn test_tbscert_unique_identifiers() {
    let mut parser = X509CertificateParser::new().with_deep_parse_extensions(false);
    let (_, x509) = parser
        .parse(Input::from(UNIQUE_IDS_DER))
