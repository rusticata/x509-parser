...
fn generalname_to_string(gn: &GeneralName) -> String {
    match gn {
        ...
        GeneralName::Invalid(any) => {
            format!("Invalid:tag={},data={:?}", any.tag(), any.data.as_bytes2())
        }
    }
}
    ...
    match ext.parsed_extension() {
        ParsedExtension::AuthorityKeyIdentifier(aki) => {
            ...
            if let Some(serial) = &aki.authority_cert_serial {
                println!("        Cert Serial: {}", format_serial(serial.as_ref()));
            }
        }
        ...
        ParsedExtension::SubjectKeyIdentifier(subject_key_id) => {
            println!(
                "      X509v3 Subject Key Identifier: {:x}",
                subject_key_id.0
            );
        }
        x => println!("      {x:?}"),
        ...
        Ok(sig_alg) => {
            ...
            match sig_alg {
                ...
                SignatureAlgorithm::RSASSA_PSS(params) => {
                    ...
                    println!("{indent_s}Salt Length: {}", params.salt_length());
                }
